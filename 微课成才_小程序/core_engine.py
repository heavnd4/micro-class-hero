...
class VideoProcessor:
    def __init__(self, base_dir=None, api_key=None):
        ...
        self.base_url = os.environ.get("DASHSCOPE_BASE_URL", "https://coding.dashscope.aliyuncs.com/apps/anthropic")
        self.model_name = os.environ.get("AI_MODEL_NAME", "qwen3.5-plus")
        ...
        # 4. 初始化 AI 客户端（轻量，可立即创建）
        self.client = anthropic.Anthropic(
            api_key=self.api_key,
            base_url=self.base_url
        )

        # 5. 重型模型延迟加载（whisper + paddleocr）
    ...
    def warmup(self):
        """预热：提前加载所有重型模型，避免首次炼化时等待"""
        print("🔥 开始预热引擎...")
        self.current_step = "预热中"
        _ = self.whisper_model   # 触发 whisper 加载
        _ = self.ocr_engine       # 触发 OCR 加载
        self.current_step = "就绪"
        print("🔥 预热完成，引擎已就绪！")
    ...
    def transcribe(self, video_path: Path, out_dir: Path):
        ...
        segments, _ = self.whisper_model.transcribe(str(video_path), language="zh")
